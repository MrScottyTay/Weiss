...
public class Weiss
{
    ...
    public static void main(String[] args)
    {
        ...
        //router1.addNodeMonitor(new NodeMonitor(router1.getName()));
        router1.start();

        //Portal 1 ---
        MetaAgent portal1 = new Portal("P1", router1);
        //portal1.addNodeMonitor(new NodeMonitor(portal1.getName()));
        portal1.start();

        Agent agent1 = new Agent("A1", portal1);
        //agent1.addNodeMonitor(new NodeMonitor(agent1.getName()));
        agent1.start();

        Agent agent2 = new Agent("A2", portal1);
        //agent2.addNodeMonitor(new NodeMonitor(agent2.getName()));
        agent2.start();

        //----------------------------------------------------------------------
        //Router 2 ---
        MetaAgent router2 = new Router("R2");
        //router2.addNodeMonitor(new NodeMonitor(router2.getName()));
        router2.start();

        //Portal 2 ---
        MetaAgent portal2 = new Portal("P2", router2);
        //portal2.addNodeMonitor(new NodeMonitor(portal2.getName()));
        portal2.start();

        Agent agent3 = new Agent("A3", portal2);
        //agent3.addNodeMonitor(new NodeMonitor(agent3.getName()));
        agent3.start();

        Agent agent4 = new Agent("A4", portal2);
        //agent4.addNodeMonitor(new NodeMonitor(agent4.getName()));
        agent4.start();
